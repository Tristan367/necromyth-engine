...
**Animation split:** `MeshInstance::secondary_joints` defines a per-bone binary split — listed joint indices use `next_animation_index` clip while the rest use the primary. `compute_joint_matrices_split()` in `animation_utils.hpp` implements this with an unordered_set lookup per joint. Zero overhead when `secondary_joints` is null.

**Hitbox system:** Bone-attached collision primitives defined per-skeleton. Two collider types: `BodyColliderDef` (physics interactions) and `HitboxAttachment` (sensor bodies for detection/raycasting). `HitboxManager` (`physics/hitbox_manager.hpp`) creates Jolt kinematic sensor bodies on a dedicated `kHitbox` object layer (non-colliding), updated each frame from bone world transforms. `build_world_matrices()` in `animation_utils.hpp` has an optional `out_bone_worlds` parameter that stores the bone world transform before IBM multiplication — used by the hitbox manager for placement. Sidecar JSON (`<model>.json`) defines hitbox and body collider data with bone references by name or joint index.
